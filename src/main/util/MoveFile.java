...
e.printStackTrace();
    ...
    public  static void moveFileChenGong(String source,String name,String mubiao){

        String startPath = source+"\\"+name;
        int t = name.indexOf("-应聘");
        String pName = name.substring(t+1,name.length());
        ...
        try {
            ...
            //  System.out.println(endPath + startFile.getName());
            if (startFile.renameTo(new File(mubiao + "\\"+pName))) {
                System.out.println("File is moved successful!");
                //   log.info("文件移动成功！文件名：《{}》 目标路径：{}",fileName,endPath);
            } else {
                System.err.println("File is failed to move!");
